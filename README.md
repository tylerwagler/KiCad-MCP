...
Not all 152 tools are dumped into the LLM context. Instead:

- **9 direct tools** are always visible (open project, start session, etc.)
- **143 routed tools** are discoverable via 4 meta-tools: `list_tool_categories`, `get_category_tools`, `execute_tool`, `search_tools`
...
- **Tools** — Actions that modify state (152 tools)
