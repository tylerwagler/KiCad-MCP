...
**151 tools** | **910 tests (676 pass, 234 env-gated skips)** | **Pure Python, zero KiCad dependency for reads**
...
Not all 151 tools are dumped into the LLM context. Instead:

- **9 direct tools** are always visible (open project, start session, etc.)
- **142 routed tools** are discoverable via 4 meta-tools: `list_tool_categories`, `get_category_tools`, `execute_tool`, `search_tools`
...
- **Tools** — Actions that modify state (151 tools)
