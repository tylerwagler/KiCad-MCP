...
class SecureSubprocess:
    ...
    def _looks_like_path(self, arg: str) -> bool:
        ...
        for ext in [".kicad_pcb", ".kicad_sch", ".kicad_mod", ".kicad_pro", ".kicad_sym"]:
            if arg.endswith(ext):
                return True
        # Output files may have export extensions
    ...
    def _validate_file_path(self, path: str) -> None:
        ...
        # Allow absolute paths with export extensions (pdf, svg, gerber, etc.)
        # These are generated by export operations like "kicad-cli pcb export gerber"
        if path.startswith(("/")) and any(path.endswith(ext) for ext in EXPORT_EXTENSIONS):
            # Still reject suspicious masquerading attempts
            if self._looks_like_suspicious_path(path):
                raise SecurityError(f"Absolute paths not allowed: {path}")
            return  # Allow export output paths

        # Reject absolute paths that don't match KiCad or export extension patterns
        ...
        if path.startswith(("/", "~")):
            # Reject absolute paths not matching known patterns
            raise SecurityError(f"Absolute paths not allowed: {path}")
    ...
    def _validate_flag_value(self, value: str, flag_name: str) -> None:
        ...
        if flag_name in {"format", "units"}:
            self._validate_known_value(value, flag_name)
        elif flag_name in {"layer", "layers"}:
            # Layer can be a single layer (F.Cu) or comma-separated layers (F.Cu,Edge.Cuts)
            self._validate_layer_value(value)
        else:
    ...
    def _validate_layer_value(self, value: str) -> None:
        """Validate a layer value (single or comma-separated layer names)."""
        # Split comma-separated layers (e.g., "F.Cu,Edge.Cuts")
        layers = value.split(",")
        for layer in layers:
            layer = layer.strip()
            if not layer:
                raise SecurityError(f"Invalid layer value: empty layer name in {value!r}")
            # Check if it's a known layer or matches KiCad layer pattern
            # KiCad layer names contain letters, dots, underscores, and spaces
            # Examples: F.Cu, B.Cu, F.Paste, Edge.Cuts, In1.Cu
            if not layer.replace(".", "").replace("_", "").replace(" ", "").isalnum():
                raise SecurityError(f"Invalid layer name: {layer!r}")
            # Basic sanity check - layer names should start with a letter
            if not layer[0].isalpha():
                raise SecurityError(f"Invalid layer name: {layer!r} (must start with letter)")
    ...
    def _validate_known_value(self, value: str, flag_name: str) -> None:
        ...
        safe_values: dict[str, frozenset[str]] = {
            "format": frozenset(
                {"json", "svg", "pdf", "dxf", "gerber", "step", "vrml", "pos", "gerbers", "plain"}
            ),
